...
def _locked(session_id: str):
    ...
    while time.monotonic() < deadline:
        try:
            ...
            if not stale_swept:
                try:
                    # time.time() (wall clock), NOT time.monotonic() -- st_mtime is a wall-clock
                    # epoch timestamp, so comparing it against a monotonic (since-boot) clock produced
                    # a large negative number and this sweep never actually fired. Found by two
                    # independent source-reading reviewers during the 2026-08-10 SOTA consult
                    # (reviews/sota-evolution/SYNTHESIS.md, finding B6); Opus 5 measured the silent
                    # cost at +2-4s added to every subsequent tool call once a hook process ever died
                    # mid-lock in that session.
                    if time.time() - lock_path.stat().st_mtime > _LOCK_STALE_AFTER_S:
...
def _cli() -> int:
    ...
    p = sub.add_parser("deliver", help="MUTATES STATE: verifies every checkable pin fresh and "
                                        "evicts anything that now fails or is over budget, then "
                                        "renders. For manual testing -- the real trigger is "
                                        "hooks/pin_deliver.py. Not a read-only preview; use 'pins' "
                                        "for that.")
    p.add_argument("--json", action="store_true")
